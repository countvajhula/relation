...
(module+ test
  ...
  ;; (check-equal? (=/classes #:key odd? (list 3 5 7)) '((3 5 7)))

  ;; set
  (check-equal? (generic-set 1 2 3) '(1 2 3) "monotonically increasing")
  (check-equal? (generic-set 1 1 2) '(1 2) "monotonically nondecreasing")
  (check-equal? (generic-set 3 3 3) '(3) "equal")
  (check-equal? (generic-set 3 3 1) '(1 3) "monotonically nonincreasing")
  (check-equal? (generic-set 3 2 1) '(1 2 3) "monotonically decreasing")
  (check-equal? (generic-set 3 2 4) '(2 3 4) "disordered")
  (check-equal? (generic-set 3) '(3) "trivial case")
  (check-exn exn:fail?
             (lambda ()
               (generic-set)) "generic relations require at least one argument")
  (check-equal? (generic-set "apple" "banana" "cherry") '("apple" "banana" "cherry"))
  (check-equal? (generic-set "apple" "apple" "apple") '("apple"))
  (check-equal? (generic-set "cherry" "banana" "apple") '("apple" "banana" "cherry"))
  (check-equal? (generic-set #"apple" #"banana" #"cherry") '(#"apple" #"banana" #"cherry"))
  (check-equal? (generic-set #"apple" #"apple" #"apple") '(#"apple"))
  (check-equal? (generic-set #"cherry" #"banana" #"apple") '(#"apple" #"banana" #"cherry"))
  (check-equal? (generic-set #\a #\b #\c) '(#\a #\b #\c))
  (check-equal? (generic-set #\a #\a #\a) '(#\a))
  (check-equal? (generic-set #\c #\b #\a) '(#\a #\b #\c))
  (check-equal? (generic-set (set) (set 1) (set 1 2)) (list (set) (set 1) (set 1 2)))
  (check-equal? (generic-set (set 1 2) (set 1 2) (set 1 2)) (list (set 1 2)))
  (check-equal? (generic-set (set 1 2) (set 1) (set)) (list (set) (set 1) (set 1 2)))
  (check-equal? (generic-set (set 1 2) (set 1 3)) (list (set 1 2) (set 1 3)) "incomparable sets")
  (check-equal? (generic-set (set 1 2) (set 3 4)) (list (set 1 2) (set 3 4)) "incomparable sets")
  (check-equal? (generic-set #:key string-length "z" "yy" "xxx") '("z" "yy" "xxx"))
  (check-equal? (generic-set #:key string-length "xxx" "zzz" "yyy") '("xxx"))
  (check-equal? (generic-set #:key string-length "xxx" "yy" "z") '("z" "yy" "xxx"))
